...
class RouteMenuButton<ButtonProps extends { route: LocationDescriptor }> extends React.Component<ButtonProps> {
  render() {
    return (
      // see https://github.com/mui-org/material-ui/issues/850#issuecomment-334118855 for 'Link' explanation.
      <MenuButton component={(linkProps: LinkProps) => <Link {...linkProps} to={this.props.route} />} {...this.props}>
        {this.props.children}
      </MenuButton>
    )
  }
...
class MenuButton extends React.Component<ButtonProps> {
  render() {
    return (
      <Button style={{ color: 'white' }} variant='text' {...this.props}>
        {this.props.children}
      </Button>
    )
  }
...
const App = (props: { classes: any }) => {
  ...
  const routeMenuButton = (route: LocationDescriptor, title: React.ReactNode) => (
    <RouteMenuButton className={classes.button} route={route}>
      {title}
    </RouteMenuButton>
  )
          ...
          <div className={classes.root}>
            ...
            <AppBar position='static'>
              <Toolbar variant='dense'>
                ...
                <MenuButton className={classes.button} target='_blank' href='https://github.com/devstronomy/'>
