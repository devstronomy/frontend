 ...
 */
const DatasetsPage = () => {
  useEffect(() => ReactGA.pageview('/datasets'))
  return (
    <S.DatasetsPage>
      ...
      <p>
        See also official{' '}
        <a href='https://nssdc.gsfc.nasa.gov/planetary/factsheet/planetfact_notes.html'>Planetary Fact Sheet Notes</a>{' '}
  ...
  mysql -u [uname] -p[pass] devstronomy &lt; data/sql/devstronomy.sql`}</pre>
     ...
     LIMIT 10`}</pre>
      ...
      <p>
        Note: the <code>planet.number_of_moons</code> does not reflect the number of records in the{' '}
        <code>satellite</code> table for Jupiter and Saturn. See the SQL select below. (<em>TODO</em>: explain why)
      </p>
